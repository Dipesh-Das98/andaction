...
const VideosSocialMedia: React.FC<VideosSocialMediaProps> = ({
  ...
  const connectYouTube = async () => {
    try {
      // ⚠️ Placeholder ID (replace with real OAuth later)
      const mockChannelId = `yt_${Date.now()}`;
      ...
      // Update local form + parent form
      setFormData(prev => ({ ...prev, ...updated }));
      onUpdateData(updated);

      // Save to backend
      await fetch("/api/artists/social/save", {
        method: "POST",
  ...
  const connectInstagram = async () => {
    try {
      // ⚠️ Placeholder ID (replace with real login later)
      const mockAccountId = `ig_${Date.now()}`;
      ...
      // Update local form + parent state
      setFormData(prev => ({ ...prev, ...updated }));
      onUpdateData(updated);
